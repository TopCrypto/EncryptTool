...
unsigned int I(unsigned int x, unsigned y, unsigned int z)
{
...
#define ROUND(F, a, b, c, d, k, s, i)   a = (a + F( b, c, d ) + x[k] + (unsigned int) (BASE_T * fabs(sin((double) i))));  \
	a = ( a<<s ) | ( a>> (32 - s )); \
	a += b;
...
int md5_hash(const unsigned char *input, int len, unsigned int hash[MD5_RESULT_SIZE])
{
	...
	//xxx should verify that len < 2^64, but since len is only 32 bits, this won't be a problem.
